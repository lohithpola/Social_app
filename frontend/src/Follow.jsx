...
const Follow = () => {
  ...
  useEffect(() => {
    if (userId !== 0) { // only fetch when userId is actually set
  ...
  }, [userId,followFlag]); // runs only when userId changes and is not 0
  

  const handleFollow=async(id,content)=>{
    if(content=="Follow"){
        try{
            const response3=await axios.post("http://localhost:8080/setFollow",{followerId:userId,followingId:id},{
                headers:{
                    Authorization:`Bearer ${jwt}`
                }
            })
    ...
    <Box sx={{ maxWidth: 600, margin: "20px auto" }}>
      {userData.map((user, index) => (
        ...
        >
          ...
          <Button onClick={(e)=>handleFollow(user.id,e.currentTarget.textContent)}
            variant="contained"
            sx={{
              textTransform: "none",
              borderRadius: 3,
